            ...
            packet format: [03]
                    ...
                    notes: U = username field
                           ...
                           notes: if alias is left blank, alias on first endpoint will be assigned the base username
                                  if left blank on multiple endpoint, server will generate a default alias
                 ...
                 examples: (03 FF 03 03 03 03 00) = authenticated
                           (03 FF 03 02 03 01 00) = username valid, alias invalid, password valid, endianess cannot be empty
                           (03 FF 06 05 05 05 00) = user already validated, password, alias and endianess unchecked
    OR
...
U10. change user alias
        ...
        S11. valid, invalid request to change user alias
                packet format: [0B]
                               [FF]
                               [ALIAS 01|02|03|07]
                               [00]
                               
                        bytes: (0B FF 01|02|03|07 00)
...
S12. update user alias
		packet format: [0C]
                ...
                bytes: (0C FF [..] FF [..] FF [..] 00)

!END: USER INFORMATION UPDATE PACKETS

//----------------------------------------------------
//PACKETS 13-39 are reserved...
...
4-BYTE CLIENT MESSAGE ID
   ...
   - The server will assign new ID's to all valid appropriate packets for retransmission.
   - These only matter for replies to the client when something goes wrong so the client can identify the packet.
...
U87. long chat message head
    ...
    S88. valid, invalid, missing long chat message head packet data
             packet format: [58]
                     ...
                     bytes: (58 FF [..] FF 01|02|03 01|02|03|04 01|03|04|11|12 00)
...
U91. long chat message next
    ...
    S92. valid, invalid, missing long chat message next packet data
             packet format: [5C]
                     ...
                     bytes: (5C FF [..] FF 01|02|03 01|02|03|08|09 01|03|04|11|12 00)
                     notes: PN = packet #
    ...
    U99. resend long chat message next packet
        ...
        S100. valid, invalid, missing resend long chat message next packet data
                 packet format: [64]
                         ...
                         bytes: (64 FF [..] FF [..] FF 01|02|03 01|02|03|08|09 01|02|03|04|11|12 00)
                         notes: PN = packet #
                                possible values: 01 = empty field
                                                 02 = invalid
                                                 03 = okay
...
!START: ROOM ACCESS, ADMIN AND OTHER CONTROL PACKETS
...
U160. join room
        packet format: [A0]
                ...
                notes: when joining a room you can leave the password blank
                       this field is only needed for rooms that are password protected
        
        server duties: if a room does not exist, create a default room with transient state
                       for the user to join.  follows IRC.  the room can then be state
                       changed by the first user that joined the room or appointed moderators.
                       first user is always the room owner.
...
U169. invite user to room
        packet format: [A9]
                ...
                notes: if alias is left blank then all endpoints associated with username should be sent the invite
                       or if alias is specififed, only the specific aliased endpoint.
                       
		REPLIES:
		
		U170. valid, invalid invite user to room request
                packet format: [A4]
                               [FF]
                               [8-BYTE UNIQUE ROOM ID]
                               [FF]
